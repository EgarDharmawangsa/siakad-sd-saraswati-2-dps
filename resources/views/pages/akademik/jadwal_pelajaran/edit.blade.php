        ...
        <form action="{{ route('jadwal-pelajaran.update', $jadwal_pelajaran->id_jadwal_pelajaran) }}" method="POST">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <select class="form-select @error('kegiatan') is-invalid @enderror" id="kegiatan"
                        name="kegiatan" required>
                        <option value="">-- Pilih Kegiatan --</option>
                        <option value="Belajar" {{ old('kegiatan', $jadwal_pelajaran->kegiatan) === 'Belajar' ? 'selected' : '' }}>
                            Belajar</option>
                        <option value="Istirahat" {{ old('kegiatan', $jadwal_pelajaran->kegiatan) === 'Istirahat' ? 'selected' : '' }}>
                            Istirahat</option>
                    </select>
                    @error('kegiatan')
                        <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" class="form-control @error('jam_mulai') is-invalid @enderror" id="jam-mulai"
                        ...
                        value="{{ old('jam_mulai', $jadwal_pelajaran->getFormatedJam('jam_mulai')) }}" required>
                    @error('jam_mulai')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" class="form-control @error('jam_selesai') is-invalid @enderror" id="jam-selesai"
                        ...
                        value="{{ old('jam_selesai', $jadwal_pelajaran->getFormatedJam('jam_selesai')) }}" required>
                    @error('jam_selesai')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
