...
class EkstrakurikulerController extends Controller
{
    ...
    public function index()
    {
        if (Gate::any(['staf-tata-usaha', 'guru'])) {
            ...
            $ekstrakurikuler = Ekstrakurikuler::withWhereHas('pesertaEkstrakurikuler', fn($query) => $query->where('id_siswa', Auth::user()->siswa->id_siswa))->filter(request()->all())->paginate(20)->withQueryString();
        } else {
            abort(404);
        }
